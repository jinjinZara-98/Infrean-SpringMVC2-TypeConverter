...
//불린 타입을 숫자로 바꾸는 것 같은 범용 기능보다는 개발자 입장에서는 문자를 다른 타입으로 변환하거나, 다른 타입을 문자로 변환하는 상황이 대부분이다.
...
public class MyNumberFormatter implements Formatter<Number> {
    ...
    //parse() 를 사용해서 문자를 숫자로 변환한다. 참고로 Number 타입은 Integer , Long 과 같은 숫자타입의 부모 클래스이다.
    ...
    public Number parse(String text, Locale locale) throws ParseException {
        ...
        //직접 할 수도 있지만 너무 오래 걸리므로 자바에 만들어져 있는걸 사용

        //locale정보를 얻어온걸 NumberFormat 객체에 담음
        //"1,000" -> 1000
        NumberFormat format = NumberFormat.getInstance(locale);

        //locale정보를 얻어온 객체에 값 text 넣어
        //text = "1,000"
